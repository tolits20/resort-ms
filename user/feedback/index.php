...
// Prepare and execute the SQL query securely for user-specific feedback
$sql_user_feedback = "SELECT f.*, a.username, b.account_id, r.room_type, r.room_code 
                      FROM feedback f 
                      JOIN booking b ON f.book_id = b.book_id 
                      JOIN account a ON b.account_id = a.account_id 
                      JOIN room r ON b.room_id = r.room_id 
                      WHERE b.account_id = ?";
$stmt_user_feedback = $conn->prepare($sql_user_feedback);

if (!$stmt_user_feedback) {
    die("Query preparation failed: " . $conn->error); // Debugging: Show SQL error
}

$stmt_user_feedback->bind_param("i", $account_id);
$stmt_user_feedback->execute();
$result_user_feedback = $stmt_user_feedback->get_result(); // Fetch result from prepared statement
    ...
    <div class="main-content">
        <div class="container">
            <div class="feedback-container">
                ...
                <?php if ($result_user_feedback->num_rows > 0): ?>
                    <?php while ($row = $result_user_feedback->fetch_assoc()): ?>
                        <div class="feedback-card">
                            <div class="feedback-header">
                                <h5>Room <?php echo htmlspecialchars($row['room_code']); ?></h5>
                                <a href="edit.php?feedback_id=<?php echo $row['feedback_id']; ?>" class="btn btn-edit">
                    ...
                    <?php endwhile; ?>
                <?php else: ?>
                    <div class="alert alert-info text-center">You haven't provided any feedback yet.</div>
                <?php endif; ?>
            </div>
